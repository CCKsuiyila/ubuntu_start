...
int DecodeIPPack(char *buf,int iBufSize){
	...
	//显示总长度
	int iIphToatl_len = pIpheader->total_len;
	//显示标识
	int iIphIdent = pIpheader->ident;
	//显示标志
	...
	//显示头部检验和
	int iIphChecksum = pIpheader->checksum;
	//显示源ip,目的ip
	...
	if(start!=NULL&&end!=NULL){
		...
		if(iIphProto==6){
			...
			//显示序列号
			int iTcphSequence = pTcpheader->sequence;
			//显示确认号
			int iTcphConfirm = pTcpheader->confirm;
			//显示首部长度
			int iTcphLen = sizeof(unsigned long)*((pTcpheader->th_lenres)>>4 &0xf);
			//保留字就不打印了
			//显示六位标志位
			...
			//显示窗口大小
			int iTcphWindowsize = pTcpheader->windowsize;
			//显示检验和
			int iTcphChecksum = pTcpheader->checksum;
			//显示紧急指针
			int iTcphUrgent_p = pTcpheader->urgent_p;
			printf("这是一个TCP包\n");
			printf("显示tcp头部信息:\n");
			printf("源端口号: %d\n",iTcphSourcePORT);
			printf("目的端口: %d\n",iTcphSdentPORT);
			...
			//显示总长度
			int iUdphLen = pUdpheader->h_len;
			//显示首部检验和
			int iUdphChecksum = pUdpheader->checksum;
			printf("这是一个Udp包\n");
			printf("显示udp头部信息\n");
			printf("源端口号: %d\n",iUdphSourcePORT);
			printf("目的端口: %d\n",iUdphSdentPORT);

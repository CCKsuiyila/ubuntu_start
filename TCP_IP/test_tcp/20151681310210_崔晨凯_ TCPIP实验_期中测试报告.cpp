...
int DecodeIPPack(char *buf,int iBufSize){
	...
	if(iIphProto==1){
		...
		//
		//
		if(type==8&&code==0&&memcmp(szDestIP,inet_ntoa(sa1.sin_addr),strlen(szDestIP))==0){
			printf("类型: 探测本机的icmp包 第 %d 个\n",count_check);
			printf("本主机地址: %s\n",inet_ntoa(sa1.sin_addr));
			printf("源地址: %s\n",szSourceIP);
			printf("目的地址: %s\n\n",szDestIP);
			++count_check;
		}else if(type==0&&code==0&&memcmp(szSourceIP,inet_ntoa(sa1.sin_addr),strlen(szDestIP))==0){

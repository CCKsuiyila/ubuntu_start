...
public class hopper_three extends Applet {
    ...
    public hopper_three() {
        ...
        Panel p = new Panel();
    	p.add(new Label("name:cck, number:20151681310210,   hopper(漏斗)"));
    ...
    public static void main(String argv[]) {
        new MainFrame(new hopper_three(), 800, 800);
    }
    ...
    class SurfaceDisplay extends Shape3D {
        ...
        Geometry createGeometry0(){
            ...
            //	
            float[] Xp0={-1.5f,-1.2f,-0.2f,-0.08f,0.0f};
            float[] Yp0={3.5f,3.0f,2.5f,0.4f,0.0f};
            //		
            ...
            TriangleArray SurfaceTriangleArray=new TriangleArray(6*n0*3, TriangleArray.COORDINATES|TriangleArray.NORMALS);
            c=0;//
            for(i=0;i<3;i++){
                for(j=0;j<n0;j++){
            ...
            for(i=0;i<3;i++){
                for(j=0;j<n0;j++){
                    Point3f A=new Point3f(SurfacePointsxyz[i][j][0],SurfacePointsxyz[i][j][1],SurfacePointsxyz[i][j][2]);
                    Point3f B=new Point3f(SurfacePointsxyz[i][j+1][0],SurfacePointsxyz[i][j+1][1],SurfacePointsxyz[i][j+1][2]);
                    Point3f C=new Point3f(SurfacePointsxyz[i+1][j+1][0],SurfacePointsxyz[i+1][j+1][1],SurfacePointsxyz[i+1][j+1][2]);
                    //
                    Vector3f a = new Vector3f(A.x - B.x, A.y - B.y, A.z - B.z);
                    Vector3f b = new Vector3f(C.x - B.x, C.y - B.y, C.z - B.z);
